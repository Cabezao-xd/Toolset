...
async function initializeCharacterCreator() {
    ...
    try {
        ...
        // Cargas opcionales: si fallan (p.ej. TS.localStorage outOfOrderMessage),
        // no deben romper la inicialización del creador.
        try { await loadLanguageData(); }   catch (e) { console.warn('[CC] loadLanguageData falló:', e); }
        try { await loadSpellDataFiles(); } catch (e) { console.warn('[CC] loadSpellDataFiles falló:', e); }
...
async function loadLanguageData(){
    try {
        ...
        console.warn('[CC] loadDataFromGlobalStorage("language") falló:', e);
        languageData = null;
    }
    ...
    console.log('[CC] savedLanguage:', savedLanguage);
}
let globalFileSize = 0;

// Retrieve data from global storage
// Intenta primero TS.localStorage; si falla (outOfOrderMessage, no disponible,
// etc.) cae a window.localStorage. Así CharacterCreator no se rompe cuando
// se abre como página separada del entryPoint.
function loadDataFromGlobalStorage(dataType) {
    return new Promise((resolve) => {
        // --- Intento 1: TS.localStorage ---
        try {
            if (typeof TS !== 'undefined' && TS?.localStorage?.global?.getBlob) {
                TS.localStorage.global.getBlob()
                    .then((data) => {
                        if (data) {
                            globalFileSize = getVariableSize(data); // Blob size in bytes
                            const allData = typeof data === 'string' ? JSON.parse(data) : data;
                    ...
                    .catch((error) => {
                        console.warn('[CC] TS.localStorage.global.getBlob falló:', error?.message || error);
                        // Fallback
                        resolve(loadFromWindowLocalStorage(dataType));
                    });
                return;
            }
        } catch (e) {
            console.warn('[CC] TS.localStorage no disponible:', e?.message || e);
        }

        // --- Intento 2: window.localStorage ---
        resolve(loadFromWindowLocalStorage(dataType));
    });
}

function loadFromWindowLocalStorage(dataType) {
    try {
        const raw = window.localStorage.getItem(`ts5e:${dataType}`);
        if (!raw) return {};
        const parsed = JSON.parse(raw);
        return parsed?.[dataType] ?? parsed ?? {};
    } catch (e) {
        console.warn('[CC] window.localStorage fallback falló:', e?.message || e);
        return {};
    }
}
...
function goBack() {
    // Intenta volver a la página anterior (v2.html).
    // Si no hay historial (porque se abrió directamente como URL),
    // navega explícitamente a v2.html.
    if (window.history.length > 1) {
        window.history.back();
        // Fallback: si en 300ms no cambió la URL, forzamos navegación
        setTimeout(() => {
            if (document.location.href.includes('CharacterCreator')) {
...
window.goBack = goBack; // por si se llama desde otro contexto